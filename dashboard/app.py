...
@st.cache_data(ttl=300)
def load_by_country():
    """Charge les statistiques par pays - avec gestion table manquante"""
    try:
        conn = get_connection()
        return pd.read_sql("""
        ...
        # Table n'existe pas → retourne DataFrame vide avec bonnes colonnes
        return pd.DataFrame(columns=['country', 'iso3', 'total_disasters', 'ongoing_count',
                                     'earthquake_count', 'flood_count', 'cyclone_count',
                                     'drought_count', 'volcano_count', 'wildfire_count',
                                     'total_population_affected', 'max_alert_level', 'latest_event_date'])

@st.cache_data(ttl=300)
def load_timeline():
    """Charge la timeline - avec gestion table manquante"""
    try:
        conn = get_connection()
        return pd.read_sql("""
        ...
        # Table n'existe pas → retourne DataFrame vide avec bonnes colonnes
        return pd.DataFrame(columns=['event_day', 'event_type', 'event_type_label',
                                     'disaster_count', 'population_affected',
                                     'red_alerts', 'orange_alerts', 'green_alerts'])
